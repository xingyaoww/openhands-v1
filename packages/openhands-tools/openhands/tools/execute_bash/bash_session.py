...
class BashSession:
    ...
    def _handle_completed_command(
        ...
        assert len(ps1_matches) >= 1, f"Expected at least one PS1 metadata block, but got {len(ps1_matches)}.\n---FULL OUTPUT---\n{pane_content!r}\n---END OF OUTPUT---"
    ...
    def _handle_nochange_timeout_command(
        ...
        if len(ps1_matches) != 1:
            logger.warning(f"Expected exactly one PS1 metadata block BEFORE the execution of a command, but got {len(ps1_matches)} PS1 metadata blocks:\n---\n{pane_content!r}\n---")
    ...
    def execute(self, action: ExecuteBashAction) -> ExecuteBashObservation:
        ...
        if len(splited_commands) > 1:
            return ExecuteBashObservation(
                output=(f"ERROR: Cannot execute multiple commands at once.\nPlease run each command separately OR chain them into a single command via && or ;\nProvided commands:\n{'\n'.join(f'({i + 1}) {cmd}' for i, cmd in enumerate(splited_commands))}"),
                error=True,
            )
        ...
        while True:
            ...
            logger.debug(f"CHECKING NO CHANGE TIMEOUT ({self.no_change_timeout_seconds}s): elapsed {time_since_last_change}. Action blocking: {is_blocking}")
            if not is_blocking and time_since_last_change >= self.no_change_timeout_seconds:
